...
foreach ([
    'css' => function($value, $key, $attr) use($state) {
        ...
        return HTML::unite('link', false, array_replace_recursive($attr, [
    ...
    'js' => function($value, $key, $attr) use($state) {
        ...
        return HTML::unite('script', "", array_replace_recursive($attr, [
    ...
    Asset::_('.' . $v, function($value, $key, $attr) use($state) {
        ...
        if ($path === false) {
            if (strpos($url, '://') !== false || strpos($url, '//') === 0) {
                return HTML::unite('img', false, array_replace_recursive($attr, [
                    'alt' => "",
                    'src' => $url
                ]));
            }
