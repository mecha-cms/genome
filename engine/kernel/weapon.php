...
class Weapon extends __ {
    ...
    protected static $armaments = [];
    protected static $armaments_x = [];
     ...
     *  --------- | ------ | ----------------------------------------
     *  $name     | string | Hook name
     *  $fail | mixed  | Fallback value if hook does not exist
     *  --------- | ------ | ----------------------------------------
     * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     *
     */
    ...
    public static function exist($name = null, $fail = false) {
        $c = static::class;
        if (is_null($name)) {
            return !empty(self::$armaments[$c]) ? self::$armaments[$c] : $fail;
        }
        return isset(self::$armaments[$c][$name]) ? self::$armaments[$c][$name] : $fail;
    }
    ...
    public static function ejected($name = null, $stack = null, $fail = false) {
        $c = static::class;
        $stack = !is_null($stack) ? $stack : 10;
        if (is_null($name)) {
            return !empty(self::$armaments_x[$c]) ? self::$armaments_x[$c] : $fail;
        } else if (is_null($stack)) {
            return !empty(self::$armaments_x[$c][$name]) ? self::$armaments_x[$c][$name] : $fail;
        }
        return isset(self::$armaments_x[$c][$name][$stack]) ? self::$armaments_x[$c][$name][$stack] : $fail;
    }
