...
class Widget extends __ {
    ...
    public static function tag($type = 'LIST', $order = 'ASC', $sorter = 'name', $max_level = 6, $folder = ARTICLE) {
        ...
        if ($files = call_user_func('Get::' . $p . 's')) {
            ...
            foreach (array_count_values($counters) as $id => $count) {
                $unit = call_user_func('Get::' . $p . 'Tag', 'id:' . $id);
                if ($unit && $id !== 0) {
                    $units[] = array(
                        'id' => $id,
                        'name' => $unit->name,
            ...
            if ( !empty($units)) {
                $units = Mecha::eat($units)->order($order, $sorter)->vomit();
                if ($type === 'LIST') {
                    $html .= $T1 . '<ul>' . NL;
                    foreach ($units as $unit) {
                        $html .= $T2 . '<li' . ($query === $unit['slug'] ? ' class="' . self::$config['classes']['current'] . '"' : "") . '><a href="' . Filter::colon('tag:url', $config->url . '/' . $config->tag->slug . '/' . $unit['slug']) . '" rel="tag">' . $unit['name'] . '</a> <span class="counter">' . $unit['count'] . '</span></li>' . NL;
                    }
                    $html .= $T1 . '</ul>' . NL;
                }
                if ($type === 'CLOUD') {
                    $units_counter = [];
                    foreach ($units as $unit) {
                        $units_counter[] = $unit['count'];
                    }
                    $highest_count = max($units_counter);
                    $_html = [];
                    foreach ($units as $unit) {
                        $size = ceil(($unit['count'] / $highest_count) * $max_level);
                        $_html[] = '<span class="size size-' . $size . ($query === $unit['slug'] ? ' ' . self::$config['classes']['current'] : "") . '"><a href="' . Filter::colon('tag:url', $config->url . '/' . $config->tag->slug . '/' . $unit['slug']) . '" rel="tag">' . $unit['name'] . '</a> <span class="counter">' . $unit['count'] . '</span></span>';
                    }
                ...
                if ($type === 'DROPDOWN') {
                    $html .= $T1 . '<select>' . NL . ($query === "" ? $T2 . '<option disabled selected>' . $speak->select . '&hellip;</option>' . NL : "");
                    foreach ($units as $unit) {
                        $html .= $T2 . '<option value="' . Filter::colon('tag:url', $config->url . '/' . $config->tag->slug . '/' . $unit['slug']) . '"' . ($query === $unit['slug'] ? ' selected' : "") . '>' . $unit['name'] . ' (' . $unit['count'] . ')</option>' . NL;
                    }
                    $html .= $T1 . '</select>' . NL;
                }
            } else {
    ...
    public static function exist($kin = null, $fail = false) {
        return self::kin($kin, $fail);
    }

    // Show the added widget ...
    public static function __callStatic($kin, $arguments = []) {
        $c = static::class;
        if ( !isset(self::$_[$c][$kin])) {
