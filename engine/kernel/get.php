...
class Get extends __ {
    ...
    public static function state_config($key = null, $fail = []) {
        $d = WORKER . DS . 'repair.state.config.php';
        $config = file_exists($d) ? include $d : $fail;
        if ($file = File::exist(STATE . DS . 'config.txt')) {
            Group::extend($config, File::open($file)->unserialize());
        }
        $config = Filter::apply('state:config', $config);
        if ( !is_null($key)) {
            return Mecha::GVR($config, $key, $fail);
        }
        return $config;
    }

    // Get stored custom field data (internal only)
    public static function state_field($key = null, $fail = [], $all = true, $scope = null) {
        $config = Config::get();
        $speak = Config::speak();
        $d = WORKER . DS . 'repair.state.field.php';
        $field = file_exists($d) ? include $d : $fail;
        if ($file = File::exist(STATE . DS . 'field.txt')) {
            Group::extend($field, File::open($file)->unserialize());
        }
        if ($all) {
            if ($e = File::exist(SHIELD . DS . $config->shield . DS . 'workers' . DS . 'fields.php')) {
                $field_e = include $e;
                Group::extend($field, $field_e);
            }
        ...
        if ( !is_null($key)) {
            return Mecha::GVR($field, $key, $fail);
        }
        // No filter
        return $field;
    }
    ...
    public static function state_menu($key = null, $fail = [], $all = true) {
        $config = Config::get();
        $speak = Config::speak();
        $d = WORKER . DS . 'repair.state.menu.php';
        $menu = file_exists($d) ? include $d : $fail;
        ...
        // Filter output(s) by `key`
        if ( !is_null($key)) {
            return Mecha::GVR($menu, $key, $fail);
        }
        // No filter
        return $menu;
    }
    ...
    public static function state_shortcode($key = null, $fail = [], $all = true) {
        $config = Config::get();
        $speak = Config::speak();
        $d = WORKER . DS . 'repair.state.shortcode.php';
        $shortcode = file_exists($d) ? include $d : $fail;
        ...
        if ($all) {
            if ($e = File::exist(SHIELD . DS . $config->shield . DS . 'workers' . DS . 'shortcodes.php')) {
                ...
                Group::extend($shortcode, $shortcode_e);
            }
            foreach (glob(PLUGIN . DS . '*' . DS . '{launch,launch__,__launch}.php', GLOB_NOSORT | GLOB_BRACE) as $active) {
        ...
        // Filter output(s) by `key`
        if ( !is_null($key)) {
            return Mecha::GVR($shortcode, $key, $fail);
        }
        // No filter
        return $shortcode;
    }
    ...
    public static function state_tag($id = null, $fail = [], $all = true, $scope = null) {
        $config = Config::get();
        $speak = Config::speak();
        $d = WORKER . DS . 'repair.state.tag.php';
        $unit = file_exists($d) ? include $d : $fail;
        if ($file = File::exist(STATE . DS . 'tag.txt')) {
            Group::extend($unit, File::open($file)->unserialize());
        }
        if ($all) {
            if ($e = File::exist(SHIELD . DS . $config->shield . DS . 'workers' . DS . 'tags.php')) {
                $unit_e = include $e;
                Group::extend($unit, $unit_e);
            }
            foreach (glob(PLUGIN . DS . '*' . DS . '{launch,launch__,__launch}.php', GLOB_NOSORT | GLOB_BRACE) as $active) {
        ...
        unset($unit_alt);
        $unit = Filter::apply('state:tag', Converter::strEval($unit));
        // Filter output(s) by `id`
        if ( !is_null($id)) {
            return Mecha::GVR($unit, $id, $fail);
        }
        // No filter
        return $unit;
    }
    ...
    public static function tags($order = 'ASC', $sorter = 'name', $scope = null) {
        $units = self::state_tag(null, [], true, $scope);
        $results = [];
        foreach ($units as $k => $v) {
            $results[] = (object) array(
                'id' => Filter::colon('tag:id', $k, $units),
                'name' => Filter::colon('tag:name', $v['name'], $units),
                'slug' => Filter::colon('tag:slug', $v['slug'], $units),
                'description' => Filter::colon('tag:description', $v['description'], $units)
            );
        }
        unset($units);
    ...
    public static function tag($filter, $output = null, $fail = false, $scope = null) {
        $units = self::tags('ASC', 'name', $scope);
        ...
        if (strpos($filter, ':') !== false) {
            ...
            foreach ($units as $k => $v) {
                if (isset($v->{$key}) && $v->{$key} === $value) {
                    return is_null($output) ? $v : (isset($v->{$output}) ? $v->{$output} : $fail);
                }
            }
    ...
    public static function postTag($filter, $output = null, $fail = false) {
        return self::tag($filter, $output = null, $fail = false, 'post');
    }
    ...
    public static function postPath($detector, $folder = POST) {
        foreach (Config::get('__' . File::B($folder) . 's_path', []) as $path) {
    ...
    public static function post($path, $excludes = [], $folder = POST, $FP = 'post:') {
        $config = Config::get();
        $speak = Config::speak();
        ...
        if ($content !== "") {
            $exc = isset($excludes['content']) && strpos($content, '<!--') !== false ? Page::text(Converter::ES($content), 'content', $FP, [], $results) : $results;
            $exc = $exc['content'];
        ...
        if ( !isset($excludes['tags'])) {
            ...
            foreach ($results['kind'] as $id) {
                $units[] = call_user_func('self::' . rtrim($FP, ':') . 'Tag', 'id:' . $id);
            }
            $results['tags'] = Filter::colon($FP . 'tags', Mecha::eat($units)->order('ASC', 'name')->vomit(), $results);
        }
        // Post Image(s)
    ...
    public static function response($path, $excludes = [], $folder = array(RESPONSE, POST), $FP = array('response:', 'post:')) {
        $config = Config::get();
        $excludes = array_flip($excludes);
        $results = false;
        if ( !is_array($path)) {
     ...
     *  $fail | string | Fallback image URL if nothing matched
     *  --------- | ------ | ----------------------------------------------------
     * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     *
     */

    public static function imagesURL($source, $fail = []) {
        $config = Config::get();
        $results = [];
        // Matched with ...
        //
        ...
        unset($results);
        return !empty($results_v) ? $results_v : $fail;
    }
    ...
    public static function imageURL($source, $sequence = 1, $fail = null) {
        $images = self::imagesURL($source, []);
        return isset($images[$sequence - 1]) ? $images[$sequence - 1] : (is_null($fail) ? Image::placeholder() : $fail);
    }
    ...
    protected static function _fields(&$results, $FP) {
        ...
        $field_d = self::state_field(null, [], true, rtrim($FP, ':'));
