...
class Mecha extends __ {
    ...
    public static function GVR(&$array, $segments = null, $fail = false) {
        if (is_null($segments)) {
            return $array;
        }
    ...
    // Vomit!BLARGH!
    public function vomit($param = null, $fail = false) {
        return self::GVR($this->stomach, $param, $fail);
    }
    ...
    public function get($index = null, $fail = false) {
        if ( !is_null($index)) {
            ...
            return array_key_exists($index, $this->stomach) ? $this->stomach[$index] : $fail;
        }
        $i = 0;
        foreach ($this->stomach as $k => $v) {
            if ($i === $this->i) {
    ...
    public function key($index, $fail = false) {
        ...
        return isset($array[$index]) ? $array[$index] : $fail;
    }
    ...
    public function chunk($index = null, $count = 25) {
        ...
        if ( !is_null($index)) {
            ...
            $this->stomach = $chunk ? array_values($chunk) : [];
