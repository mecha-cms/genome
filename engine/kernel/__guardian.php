...
class Guardian extends __ {
    ...
    public static function get($key = null, $fail = "") {
        if ($log = Session::get('cookie:' . self::$user, false)) {
            if ( !is_null($key)) {
                return isset($log[$key]) ? $log[$key] : $fail;
            }
            return $log;
        }
        return $fail;
    ...
    public static function ally($user = null, $fail = false) {
        if ($file = File::exist(LOG . DS . 'users.txt')) {
            ...
            return isset($ally[$user]) ? $ally[$user] : $fail;
        } else {
    ...
    public static function checker($name, $action) {
        ...
        self::$validator[static::class][$name] = $action;
    }
    ...
    public static function checkerExist($name = null, $fail = false) {
        $c = static::class;
        if (is_null($name)) {
            return isset(self::$validator[$c]) && !empty(self::$validator[$c]) ? self::$validator[$c] : $fail;
        }
        $name = strtolower($name);
        if (strpos($name, 'is_') !== 0) $name = 'is_' . $name;
        return isset(self::$validator[$c][$name]) ? self::$validator[$c][$name] : $fail;
    }
    ...
    public static function memorize($name = null, $value = "") {
        if (is_null($name)) {
            $name = $_SERVER['REQUEST_METHOD'] === 'POST' ? $_POST : [];
        }
        if (is_object($name)) {
            $name = Mecha::A($name);
        }
        ...
        Session::set(self::$form, Group::extend($memory, $name));
    }
    ...
    public static function wayback($name = null, $fail = "") {
        $memory = Session::get(self::$form);
        self::forget($name);
        return !is_null($name) ? Mecha::GVR($memory, $name, $fail) : $memory;
    }
    ...
    public static function math($min = 1, $max = 10, $text = [], $format = '%1$s %3$s %2$s') {
        $x = mt_rand($min, $max);
        $y = mt_rand($min, $max);
